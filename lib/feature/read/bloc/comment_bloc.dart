...
class CommentBloc extends Bloc<CommentEvent, CommentState> {
  ...
  CommentBloc({
    required this.loadCommentUsecase,
  ...
  Future<void> _onLoad(LoadCommentEvent event, Emitter emit) async {
    ...
    try {
      List<CommentEntity> comments = await loadCommentUsecase.loadAllComment(
        event.slug,
      );
  ...
  Future<void> _onSendComment(SendCommentEvent event, Emitter emit) async {
    emit(LoadingCommentState());
    try {
      CommentEntity? commentEntity = await this.sendCommentUsecase.call(
        event.slug,
        event.comment,
        event.chapter,
      );
